...
- For `rg` content-search output consumed by an agent or tool, always pass `--color=never --no-heading --with-filename --line-number` instead of relying on TTY detection; when searching for exact text, also use `-F`, and reserve regular expressions for searches that need pattern matching.
